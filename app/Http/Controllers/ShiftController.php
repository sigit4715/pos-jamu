...
class ShiftController extends Controller
{
    ...
    {
        $current = CashierShift::where('user_id', auth()->id())->where('status', 'open')->latest()->first();
    ...
    public function open(Request $request)
    {
        ...
        CashierShift::create(['user_id' => auth()->id(), 'opened_at' => now(), 'opening_cash' => $data['opening_cash'], 'status' => 'open', 'notes' => $data['notes'] ?? null]);
